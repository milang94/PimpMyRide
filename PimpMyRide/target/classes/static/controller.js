		...
		.controller(
				...
				[
						...
						function($scope, $rootScope, service, $location,
							...
							$scope.findModels = function(brandName) {
								service.findModels(brandName).then(
										function(response) {
											...
											// $window.localStorage.setItem('brandd',
											// brandName);
							...
							/*
							 * $scope.ref = function() { if ($rootScope.car ==
							 * undefined) { var b = $window.localStorage
							 * .getItem('brandd'); var m = $window.localStorage
							 * .getItem('modell'); var y = $window.localStorage
							 * .getItem('yearrr'); var m0 = $window.localStorage
							 * .getItem('mt0'); var m1 = $window.localStorage
							 * .getItem('mt1'); var m2 = $window.localStorage
							 * .getItem('mt2'); $rootScope.varvar = true;
							 * service .allInformations(b, m, y, m0, m1, m2)
							 * .then( function(response) { $rootScope.car =
							 * response.data; $rootScope.myText = "<h3>" + b + "</h3><h4>" +
							 * m + "</h4><h5>" + y + "</h5><h6>" + m0 + " " +
							 * m1 + " " + m2 + "</h6>"; }); } }
							 */
						} ]);